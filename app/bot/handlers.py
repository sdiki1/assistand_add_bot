...
SECOND_SURVEY_FOLLOWUP_MESSAGE = (
    "Анкета получена\n"
    "Спасибо, вы успешно отправили своё резюме в базу ассистентов <b>Redpradaassist 💔\n"
    "Теперь рассказываем, что будет дальше.</b>\n\n"
    "🔎 Этап 1. Проверка профиля\n"
    "Наша команда просматривает ваше резюме и результаты интервью с ботом.\n"
    "Это нужно, чтобы предлагать вас только на подходящие роли, а не отправлять резюме «<b>вслепую</b>».\n\n"
    "🗂 Этап 2. Попадание в базу\n"
    "После проверки вы попадаете в закрытую базу ассистентов.\n"
    "Мы работаем в бутиковом формате, поэтому для нас важно не количество, а точное совпадение.\n\n"
    "💼 Этап 3. Подбор вакансий\n"
    "Когда появляется подходящий запрос, мы предлагаем вакансию, которая соответствует:\n"
    "/вашему уровню/ вашему опыту/ вашему рабочему формату\n\n"
    "⏳ Сроки\n"
    "Сейчас в команде один рекрутер, поэтому процесс может занимать немного больше времени, чем в массовых агентствах.\n"
    "Но наша задача: <b>довести вас до реального трудоустройства, а не просто сохранить резюме в базе.</b>\n\n"
    "📩 Если у вас изменится опыт, навыки или формат занятости —\n"
    "вы всегда можете написать нам и обновить информацию.\n"
    "Спасибо за доверие к <b>Redpradaassist</b> 🤍"
)
...
async def start_command(message: Message) -> None:
    ...
    text = ("Если Вы смотрели фильм <b>«Дьявол носит Прада»</b> и помните успевающую во всем ассистенку, которая успевала всё — приятно познакомиться!\n\n"
        "За годы работы я узнала, как <b>«крутится каждый винтик»</b> бизнес-процессов, "
        "и получила обширный опыт в <b>fashion-retail</b>, <b>продажах</b>, <b>IT</b> и <b>управлении операционными задачами</b>. "
        "Более 7 лет я работаю в роли той самой <b>right hand</b> руководителя — и теперь помогаю другим ассистентам находить своё место рядом с сильными лидерами👠 \n\n"
        "Заполни короткую анкету, чтобы мы могли предложить тебе подходящие вакансии.")
...
async def finish_response(message: Message, session: AsyncSession, response_id: int) -> None:
    ...
    await message.answer(SECOND_SURVEY_FOLLOWUP_MESSAGE, reply_markup=ReplyKeyboardRemove(), parse_mode="HTML")
    await message.answer(FOLLOW_UP_MESSAGE, parse_mode="HTML")
